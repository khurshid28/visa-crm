// Mavjud arizachilarga (generatedPassword bo'sh bo'lganlarga) tizim parolini
// hisoblab yozadi. lib/email.ts dagi buildPassword bilan bir xil algoritm.
// Ishga tushirish: node prisma/backfill-passwords.mjs
...
async function main() {
  const rows = await prisma.applicant.findMany({
    where: { OR: [{ generatedPassword: null }, { generatedPassword: "" }] },
    select: { id: true, name: true, surname: true, passportNumber: true },
  });

  console.log(`Topildi: ${rows.length} ta parolsiz arizachi`);
  let done = 0;
  for (const a of rows) {
  ...
  console.log(`Yangilandi: ${done} ta`);
}
